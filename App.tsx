    ...
    <div className="h-screen w-screen flex flex-col bg-saga-gradient overflow-hidden select-none">
      ...
      <div className="flex-grow relative flex flex-col items-center justify-center p-2 border-b border-zinc-800/40 overflow-hidden">
        <div className="absolute inset-0 opacity-[0.02] pointer-events-none flex items-center justify-center">
            <Dices className="w-64 h-64 text-red-500" />
        </div>
              ...
              <div className="flex-grow overflow-y-auto no-scrollbar pb-32 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 h-[280px]">
                  {[
                    ...
                    { val: DifficultyLevel.CHALLENGING, label: 'Challenge' },
                    ...
                    <button key={lvl.val} onClick={() => setDifficulty(lvl.val)} className={`flex flex-col items-center justify-center rounded-[1.5rem] border-2 transition-all active:scale-95 ${difficulty === lvl.val ? 'bg-red-600 border-red-400 text-white shadow-[0_0_20px_rgba(220,38,38,0.4)]' : 'bg-zinc-950 border-zinc-800 text-zinc-500'}`}>
                      <span className="text-[11px] font-black uppercase tracking-widest leading-none mb-1">{lvl.label}</span>
                      <span className="font-saga text-lg opacity-80">{lvl.val}</span>
                    </button>
                  ))}
                </div>
              </div>
